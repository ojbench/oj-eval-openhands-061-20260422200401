...
class ACMOJClient:
    ...
    def _make_request(self, method: str, endpoint: str, data: Dict[str, Any] = None, 
        ...
        try:
            ...
            try:
                # Attempt to show server-provided error details
...
def main():
    ...
    # Submit: if --language/--code-file are omitted, submit current git repository
    submit_parser = subparsers.add_parser("submit", help="Submit code: raw text or current git repo")
    ...
    submit_parser.add_argument("--language", type=str, required=False,
                               help="Programming language for raw submission (e.g., cpp, c, python). Omit for git repo submission")
    submit_parser.add_argument("--code-file", type=str, required=False,
                               help="Path to the source code file when using raw submission")
    ...
    if args.command == "submit":
        if args.language and args.code_file:
            ...
            # submit current git repo URL
            # read origin url
            import subprocess
            try:
                origin_url = subprocess.check_output(["git", "config", "--get", "remote.origin.url"], text=True).strip()
            except subprocess.CalledProcessError:
                print("Error: Failed to get git remote URL. Ensure this is a git repo.")
                exit(1)
            result = client.submit_git(args.problem_id, origin_url)
